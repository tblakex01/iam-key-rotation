...
def load_config():
    ...
    try:
        ...
        CONFIG["NEW_KEY_RETENTION_DAYS"] = int(env_vars.get("NEW_KEY_RETENTION_DAYS", 45))
        CONFIG["OLD_KEY_RETENTION_DAYS"] = int(env_vars.get("OLD_KEY_RETENTION_DAYS", 30))
...
def test_old_key_warning():
    ...
    console.print("\n")
    console.print(Panel.fit(f"⚠️  Testing Day {warning_day} Warning (Old Key Deletion in 7 Days)", style="bold yellow"))
...
def test_old_key_deletion():
    ...
    console.print("\n")
    console.print(Panel.fit(f"🗑️  Testing Day {deletion_day} Cleanup (Old Key Deletion)", style="bold red"))
...
def test_s3_cleanup():
    ...
    console.print("\n")
    console.print(Panel.fit(f"🗑️  Testing Day {cleanup_day} S3 Cleanup (Expired Credentials)", style="bold red"))
    ...
    response = lambda_client.invoke(
        FunctionName="iam-s3-credentials-cleanup",
        InvocationType="RequestResponse"
    )
    ...
    console.print("[yellow]Check your email for the expiration notice (if not downloaded)![/yellow]")
...
def main():
    ...
    console.print(f"{len(reminder_days) + 2}. Day {deletion_day} Deletion (Old key removed)")
    
    console.print(f"\n[bold]Credential Expiration:[/bold]")
    console.print(f"{len(reminder_days) + 3}. Day {cleanup_day} S3 Cleanup (Expired credentials)")
